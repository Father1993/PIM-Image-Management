...
class AsyncCompoImageChecker:
    ...
    async def get_products_without_images(self):
        ...
        async with aiohttp.ClientSession(
            ...
            while True:
                ...
                if (
                    batch_num % 10 == 0 or batch_without_images > 0
                ):  # Показываем прогресс
                    print(
                        f"🚀 Пакет {batch_num}: {total_processed}/{total_products} товаров | Без изображений: {len(products_without_images)}"
                    )
    ...
    def save_to_excel(self, products):
        ...
        ws.cell(
            row=len(products) + 4,
            column=1,
            value=f"Всего товаров без изображений: {len(products)}",
        )
